...
| `im:message.group_msg:get_as_user` | 以用户身份读取群聊消息 |
...
# 消息
python3 $S list-groups                                           # 列出群组
python3 $S send-msg oc_xxx "消息内容"                              # 发群消息
python3 $S send-msg ou_xxx "消息内容" --user                       # 私聊消息
python3 $S send-card oc_xxx '{"type":"template","data":{}}'  # 发送交互卡片
python3 $S get-msgs oc_xxx today                                 # 读取今日消息
python3 $S get-msgs-user ou_xxx last_3_days                      # 读取近期私聊
python3 $S search-msgs "部署"                                     # 搜索消息
python3 $S reply om_xxx "收到！"                                   # 回复消息
python3 $S get-thread thread_xxx                                 # 获取话题回复

# 日历
python3 $S list-events                                           # 查看今日日程
python3 $S create-event "迭代评审" "2026-03-27T14:00" "2026-03-27T15:00"  # 所有日程自动创建视频会议
python3 $S create-event "演示会" "2026-03-28T10:00" "2026-03-28T11:00"    # 所有日程自动创建视频会议

# 多维表格
python3 $S list-tables appToken                                  # 列出表格
python3 $S list-records appToken tableId                         # 列出记录
python3 $S create-record appToken tableId '{"任务":"修复bug"}'    # 创建记录
python3 $S update-record appToken tableId recordId '{"状态":"完成"}'  # 更新记录

# 群组管理
python3 $S create-group "项目团队"                                 # 创建群组
python3 $S add-members oc_xxx '["ou_xxx1","ou_xxx2"]'            # 添加成员
```
...
| `list-groups` | 列出群组 |
| `send-msg <id> <text> [--user]` | 发送消息（群聊或私聊） |
| `send-card <id> '<json>' [--user]` | 发送交互卡片消息 |
| `get-msgs <chat_id> [time] [count]` | 读取群聊记录（today/yesterday/this_week/last_3_days 等） |
| `get-msgs-user <open_id> [time] [count]` | 读取私聊记录 |
| `search-msgs <keyword> [time]` | 跨群搜索消息 |
| `reply <msg_id> <text> [--thread]` | 回复消息（支持话题模式） |
| `get-thread <thread_id>` | 获取话题回复 |
| `list-events [date]` | 查看日程（默认今天） |
| `create-event <title> <start> <end> [attendees]` | 创建日程（自动创建视频会议） |
| `list-tables <app_token>` | 列出多维表格中的表格 |
| `list-records <app> <table> [page_size]` | 列出多维表格记录 |
| `create-record <app> <table> <json>` | 创建多维表格记录 |
| `update-record <app> <table> <record> <json>` | 更新多维表格记录 |
| `create-group <name> [members_json]` | 创建群组 |
| `add-members <chat_id> '<members_json>'` | 添加群成员 |
